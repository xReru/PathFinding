...
public class PathFinding {
	...
	//UTIL ARRAYS
	private String[] algorithms = {"Dijkstra","A*", "Depth-First", "Breadth-First", "Bellman-Ford"};
	private String[] tools = {"Start","Finish","Wall", "Eraser"};
	//BOOLEANS
	private boolean solving = false;
	private boolean solving1 = false;
	//UTIL
	Node[][] map;
	...
	class Algorithm {	//ALGORITHM CLASS
		...
		public void BellmanFord() {
			...
			while(solving) {
				...
				if(explored.size() > 0) {
					...
					// 	System.out.print(current.getType());	//IF THE NODE IS THE FINISH THEN BACKTRACK TO GET THE PATH
					// 	backtrack(current.X(), current.Y(),hops);
					// }	//ADD ALL THE NEW NODES TO THE QUE
					Update();
					delay();
				} else {	//IF NO NODES WERE EXPLORED THEN JUST REMOVE THE NODE FROM THE QUE
					priority.remove(0);
		...
		public ArrayList<Node> exploreNeighbors4(Node current, int hops) {	//EXPLORE NEIGHBORS
			...
			for(int a = -1; a <= 1; a++) {
				for(int b = -1; b <= 1; b++) {
					...
					// System.out.println("Current X,Y: "+current.getX()+", "+current.getX()+" | End Node X,Y: "+finishx+", "+finishy);
					int xbound = current.getX()+a;
					int ybound = current.getY()+b;
					if((xbound > -1 && xbound < cells) && (ybound > -1 && ybound < cells)) {	//MAKES SURE THE NODE IS NOT OUTSIDE THE GRID
						Node neighbor = map[xbound][ybound];
						if((neighbor.getHops()==-1 || neighbor.getHops() > hops) && current.getType()!= 2) {	//CHECKS IF THE NODE IS NOT A WALL AND THAT IT HAS NOT BEEN EXPLORED
		...
		public void explore3(Node current, int lastx, int lasty, int hops) {
			...
			current.setHops(hops);//SET THE HOPS FROM THE START
                        if(current.getType()==1){
                            current.setEndP(current.getX(),current.getY());
                        }
			if (lastx == cells-2 && lasty == cells-2){
                            backtrack1(current.endX, current.endY,hops);
		...
		public ArrayList<Node> exploreNeighbors1(Node current, int hops) {	//EXPLORE NEIGHBORS // DIAGONAL BFS
			ArrayList<Node> explored = new ArrayList<Node>();	//LIST OF NODES THAT HAVE BEEN EXPLORED
			for(int a = -1; a <= 1; a++) {
				for(int b = -1; b <= 1; b++) {
					int xbound = current.getX()+a; 
		...
		public ArrayList<Node> exploreNeighbors2(Node current, int hops) {	//EXPLORE NEIGHBORS // DFS
			ArrayList<Node> explored = new ArrayList<Node>();	//LIST OF NODES THAT HAVE BEEN EXPLORED
			for(int a = -1; a <= 1; a++) {
				for(int b = -1; b <= 1; b++) {
					int xbound = current.getX()+a;
		...
		public void backtrack(int lx, int ly, int hops) {	//BACKTRACK
			...
			solving = false;
                ...
                public void backtrack1(int lx, int ly, int hops) {	//BACKTRACK
			length = hops;
			while(hops > 1) {	//BACKTRACK FROM THE END OF THE PATH TO THE START
	...
	class Node {
		...
		public void setLastNode(int x, int y) {lastX = x; lastY = y;}
                public void setEndP(int x, int y) {endX = x; endY = y;}
